...
const fragmentShaders = {
  liquidGlass: (uv: Vec2, _mousePosition?: unknown, _time?: unknown): Vec2 => {
    const ix = uv.x - 0.5
    const iy = uv.y - 0.5
  ...
  liquidGlass2: (uv: Vec2, _mousePosition?: unknown, _time?: unknown): Vec2 => {
    const ix = uv.x - 0.5
    const iy = uv.y - 0.5
    const distanceToEdge = roundedRectSDF(ix, iy, 0.35, 0.25, 0.8)
    const displacement = smoothStep(0.7, 0, distanceToEdge - 0.12)
    const scaled = smoothStep(0, 1, displacement)
    return texture(ix * scaled + 0.5, iy * scaled + 0.5)
  },
  flowingLiquid: (uv: Vec2, _mousePosition?: unknown, _time?: unknown): Vec2 => {
    const ix = uv.x - 0.5
    const iy = uv.y - 0.5
  ...
  transparentIce: (uv: Vec2, _mousePosition?: unknown, _time?: unknown): Vec2 => {
    const ix = uv.x - 0.5
    const iy = uv.y - 0.5
  ...
  unevenGlass: (uv: Vec2, _mousePosition?: unknown, _time?: unknown): Vec2 => {
    const ix = uv.x - 0.5
    const iy = uv.y - 0.5
  ...
  mosaicGlass: (uv: Vec2, _mousePosition?: unknown, _time?: unknown): Vec2 => {
    const ix = uv.x - 0.5
    const iy = uv.y - 0.5
    const tileSize = 0.05
...
self.onmessage = (e: MessageEvent) => {
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      const pos = fragmentShaders[effect as FragmentShaderType](uv, mousePosition, time)
      const dx = pos.x * w - x
      const dy = pos.y * h - y
